...
contract Applesnakes is ERC721, ERC721Burnable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant UNWRAP_COOLDOWN = 1 hours;  // no instant mint/unwrap/sell bots allowed!
    ...
    mapping(uint256 => uint256) public swapMintTime;  // <-- ADD THIS
    ...
    uint256 public breedFee = 0.006969 ether;  
    ...
    */
    ...
    function swapMint() external payable nonReentrant {
        require(live == true, "swap mint is not active");
        ...
        for (uint256 i = 0; i < numWholeTokens && _swapMintId <= MAX_SWAP_MINTS; i++) {
            ...
            swapMintTime[id] = block.timestamp;
            mintedCount++;
        }
    ...
    /**
    * @notice Simple swap ETH for tokens (no vesting, no minting)
    * @dev Tokens sent directly to caller
    */
    function swap() external payable nonReentrant {
        ...
        try MANAGER.unlock(
            abi.encode(
                SwapData({
                    ...
                    zeroForOne: true,
                    amountIn: msg.value,
                    recipient: msg.sender,  // <-- Send directly to caller
                    payer: msg.sender,
                    payC0AsNative: true
                })
            )
        ...
        emit Swapped(msg.sender, msg.value, amountOut);
    }
    ...
    */
    function getTokenInfo(uint256[] calldata tokenIds) external view returns (TokenInfo[] memory infos) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            // Calculate if can unwrap
            bool canUnwrap = true;
            if (swapMintTime[tokenId] > 0) {
                canUnwrap = block.timestamp >= swapMintTime[tokenId] + UNWRAP_COOLDOWN;
            }
            ...
            infos[i] = TokenInfo({
                ...
                swapMintTime: swapMintTime[tokenId],
                canUnwrap: canUnwrap
            });
        }
    }
        ...
        returns (address) 
    {
        ...
        // Check unwrap cooldown ONLY when transferring to wrapper
        if (to == wTokens && swapMintTime[tokenId] > 0) {
            require(
                block.timestamp >= swapMintTime[tokenId] + UNWRAP_COOLDOWN,
                "Unwrap cooldown active"
            );
        }
