...
/**
 * @title PredictionMarketHub - FIXED VERSION with Frontend Helpers
 * @notice Key features:
 * 1. CRITICAL FIXES: Deposit reduction, DoS prevention, token lock protection
 * 2. FRONTEND HELPERS: Easy detection of claimable markets and user positions
 */
contract PredictionMarketHub {
    ...
    // NEW: Struct for claimable market info
    ...
    // NEW: Struct for user position info
    struct UserPosition {
        uint256 gameId;
        uint256 yesShares;
        uint256 noShares;
    ...
    event TradingFeeCollectionFailed(uint256 indexed gameId, address indexed from, uint256 amount, string reason);
    ...
    function buyShares(uint256 gameId, uint256 tokensIn, bool isYes) external nonReentrant {
        ...
        if (feeAmount > 0 && address(appleStaking) != address(0)) {
            _collectFeeToStaking(gameId, msg.sender, feeAmount, isYes ? "buyYes" : "buyNo");
        }
    ...
    function sellShares(uint256 gameId, uint256 sharesIn, bool isYes) external nonReentrant {
        ...
        if (feeAmount > 0 && address(appleStaking) != address(0)) {
            _collectFeeToStaking(gameId, msg.sender, feeAmount, isYes ? "sellYes" : "sellNo");
        }
    ...
    function claimWinnings(uint256 gameId) external nonReentrant {
        ...
        // CRITICAL FIX: Reduce deposits based on what was claimed
        if (m.result == GameResult.Win && userYes > 0 && m.yesSharesTotal > 0) {
            uint256 share = (userYes * 1e18) / m.yesSharesTotal;
            uint256 yesReduction = (m.yesDeposits * share) / 1e18;
            uint256 noReduction = (m.noDeposits * share) / 1e18;
        ...
        } else if (m.result == GameResult.Lose && userNo > 0 && m.noSharesTotal > 0) {
            uint256 share = (userNo * 1e18) / m.noSharesTotal;
            uint256 yesReduction = (m.yesDeposits * share) / 1e18;
            uint256 noReduction = (m.noDeposits * share) / 1e18;
            ...
            if (userYes > 0 && m.yesSharesTotal > 0) {
                uint256 yesReduction = (userYes * m.yesDeposits) / m.yesSharesTotal;
                m.yesDeposits = yesReduction > m.yesDeposits ? 0 : m.yesDeposits - yesReduction;
            }
            if (userNo > 0 && m.noSharesTotal > 0) {
                uint256 noReduction = (userNo * m.noDeposits) / m.noSharesTotal;
                m.noDeposits = noReduction > m.noDeposits ? 0 : m.noDeposits - noReduction;
            }
        }
        ...
        if (feeAmount > 0 && address(appleStaking) != address(0)) {
            _collectFeeToStaking(gameId, msg.sender, feeAmount, "claimWinnings");
        }
    ...
    function _collectFeeToStaking(uint256 gameId, address from, uint256 feeAmount, string memory feeType) internal {
        try IERC20Minimal(token).approve(address(appleStaking), 0) {} catch {}
        
        try IERC20Minimal(token).approve(address(appleStaking), feeAmount) returns (bool success) {
            if (success) {
                try appleStaking.addRewards(feeAmount) {
                    emit TradingFeeCollected(gameId, from, feeAmount, feeType);
                } catch {
                    uncollectedFees += feeAmount;
                    emit TradingFeeCollectionFailed(gameId, from, feeAmount, "addRewards failed");
                }
            } else {
                uncollectedFees += feeAmount;
                emit TradingFeeCollectionFailed(gameId, from, feeAmount, "approve returned false");
            }
        } catch {
            uncollectedFees += feeAmount;
            emit TradingFeeCollectionFailed(gameId, from, feeAmount, "approve reverted");
        }
    }
    ...
    /**
     * @notice Get all markets where user has claimable winnings
     * @dev Optimized for frontend - checks resolved markets only
     * @param user Address to check
     * @param maxResults Maximum number of results to return (capped at 50)
     * @return claimable Array of claimable market info
     * @return totalClaimable Total amount claimable across all markets
     */
    function getUserClaimableMarkets(address user, uint256 maxResults) 
        external 
        view 
        returns (
            ClaimableMarket[] memory claimable,
            uint256 totalClaimable
        ) 
    {
        if (maxResults > 50) maxResults = 50; // Gas limit protection
        
        // Count how many markets have claimable winnings
        ...
        return (claimable, totalClaimable);
    }
    
    /**
     * @notice Check if user has any claimable winnings (fast check)
     * @param user Address to check
     * @return hasClaimable True if user has any claimable winnings
     * @return firstClaimableGameId First market ID with claimable winnings (0 if none)
     */
    function userHasClaimable(address user) 
        external 
        view 
        returns (bool hasClaimable, uint256 firstClaimableGameId) 
    {
    ...
    /**
     * @notice Get all user's positions (active + resolved, not closed)
     * @param user Address to check
     * @param maxResults Maximum number of results (capped at 100)
     * @return positions Array of user positions
     */
    function getUserPositions(address user, uint256 maxResults)
        external
        view
        returns (UserPosition[] memory positions)
    {
        ...
        for (uint256 i = 0; i < positionCount; i++) {
            ...
            if (m.resolved) {
                ...
                // Calculate current market value for active positions
                if (userYes > 0 && m.yesSharesTotal > 0) {
    ...
    /**
     * @notice Batch check claimable amounts for multiple markets
     * @param gameIds Array of market IDs to check
     * @param user Address to check
     * @return claimableAmounts Array of claimable amounts (same order as gameIds)
     */
    function batchGetClaimable(uint256[] calldata gameIds, address user)
        external
        view
        returns (uint256[] memory claimableAmounts)
    {
    ...
    /**
     * @notice Get summary of user's portfolio
     * @param user Address to check
     * @return activePositions Number of active positions
     * @return resolvedPositions Number of resolved positions awaiting claim
     * @return totalClaimable Total claimable amount
     * @return totalActiveValue Total value of active positions
     */
        ...
        )
    {
        ...
        // Count resolved positions and claimable
        for (uint256 i = 0; i < resolvedMarketIds.length; i++) {
            uint256 gameId = resolvedMarketIds[i];
        ...
        return (activePositions, resolvedPositions, totalClaimable, totalActiveValue);
    }
