...
function TransactionCard({ transaction, chainId }: { transaction: Transaction; chainId: number }) {
  ...
  const getStatusConfig = () => {
    switch (transaction.status) {
      case 'pending':
        return {
          bg: 'bg-[rgba(255,208,117,0.08)] border-[rgba(255,208,117,0.2)]',
          text: 'text-[#ffd075]',
          icon: '\u23F3',
          label: 'Pending',
        };
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <a
              ...
              className="text-[#ffd075] hover:text-[#ffd075] text-xs flex items-center gap-1 transition-colors"
            >
        ...
        {transaction.status !== 'pending' && (
          <button
            ...
            className="text-[#8a9090] hover:text-white transition-colors flex-shrink-0"
          >
