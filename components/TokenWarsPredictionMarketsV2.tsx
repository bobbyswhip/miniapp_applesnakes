...
export function TokenWarsPredictionMarketsV2({
  ...
  if (markets.length === 0) {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center bg-[#1a2221]/30 rounded-lg p-4">
        <p className="text-[#8a9090] text-sm mb-2">No prediction markets for this war.</p>
        <div className="text-[10px] text-[#6b7575] space-y-1 text-center">
          <p>War ID: {warId}</p>
          <p className="font-mono break-all">bytes32: {bytes32Id.slice(0, 20)}...{bytes32Id.slice(-8)}</p>
          <p>Total on-chain markets: {totalMarkets}</p>
          <p className="mt-2 text-[#5a6565]">Markets are created by admin when a war starts</p>
        </div>
        <button
          onClick={refresh}
          className="mt-3 px-3 py-1 bg-[#1f2827] text-white text-xs rounded hover:bg-[#2a3533]"
        >
          Refresh
        </button>
      </div>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div className="text-right">
            <div className="text-xs font-bold text-[#ffd075]">{formatWASS(totalPool)} wASS</div>
            <div className="text-[10px] text-[#8a9090]">{market.outcomes.length} outcomes</div>
          </div>
          <svg
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <span className="text-sm font-bold text-[#ffd075]">
                      {outcome.probability.toFixed(1)}%
                    </span>
                  </div>

                  {/* Probability Bar */}
                  <div className="relative w-full h-3 rounded-full overflow-hidden bg-[#1f2827] mb-1.5">
                    <div
              ...
              <div className="flex gap-1 mb-2">
                <button
                  ...
                  className={`flex-1 py-1.5 text-[10px] font-bold rounded transition-all ${
                    paymentType === "wass"
                      ? 'bg-[#c5a97b]/30 text-[#ffd075] border border-[#c5a97b]'
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded font-bold text-[10px] transition-all ${
                    isBuying
                      ...
                      : 'bg-[#a88a5a] text-white hover:bg-[#c5a97b]'
                  }`}
                >
  ...
  return (
    <div className="mt-2 pt-2 border-t border-[rgba(255,208,117,0.15)] bg-[#3d2e1a]/10 rounded-lg p-2">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[10px] text-[#ffd075] font-bold">📊 Your Position</span>
      ...
      {/* Summary Stats */}
      <div className="grid grid-cols-2 gap-1 mb-2 text-center bg-[#1a2221]/30 rounded-lg p-1.5">
        <div>
        ...
        <div>
          <div className="text-[9px] text-[#6b7575]">Claimable</div>
      ...
      {position.positions.map((pos) => {
        ...
        return (
          <div
            ...
            className={`flex items-center justify-between p-2 bg-[#1a2221]/50 rounded-lg mb-1 border ${
              isWinner ? 'border-yellow-500/50' : 'border-[rgba(255,208,117,0.1)]'
            }`}
          >
      ...
      <div className="bg-[#1a2221] rounded-lg p-4 max-w-sm w-full border border-[rgba(255,255,255,0.06)]">
        ...
        <div className="mb-3">
          ...
          <div className="flex justify-between text-xs text-[#6b7575] mt-1">
            <button onClick={() => setSharesToSell((maxShares * 0.25).toFixed(4))} className="hover:text-[#ffd075]">25%</button>
            <button onClick={() => setSharesToSell((maxShares * 0.5).toFixed(4))} className="hover:text-[#ffd075]">50%</button>
            <button onClick={() => setSharesToSell((maxShares * 0.75).toFixed(4))} className="hover:text-[#ffd075]">75%</button>
            <button onClick={() => setSharesToSell(maxShares.toFixed(4))} className="hover:text-[#ffd075]">Max</button>
          </div>
        </div>
        ...
        <div className="mb-3 p-2 bg-green-900/30 rounded border border-green-700/30">
          ...
          {estimate && (
            <div className="text-[10px] text-[#6b7575]">
