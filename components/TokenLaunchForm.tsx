...
export function TokenLaunchForm() {
  ...
  if (!isConnected) {
    return (
      <div className="space-y-6">
        <div className="p-6 bg-[#171e1d]/50 border border-[rgba(255,255,255,0.08)] rounded-xl text-center space-y-4">
          <div className="w-16 h-16 mx-auto bg-[rgba(255,208,117,0.1)] rounded-full flex items-center justify-center">
            <svg className="w-8 h-8 text-[#c5a97b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-gradient-to-r from-[#b8860b] to-[#c5a97b] text-white font-semibold rounded-lg hover:from-[#c5a97b] hover:to-[#ffd075] transition-all"
          >
            Connect Wallet
          </button>
        </div>
        ...
        <div className="text-xs text-[#6b7575] space-y-1">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between p-3 bg-[#171e1d]/50 border border-[rgba(255,255,255,0.08)] rounded-lg">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
          <span className="text-sm text-[#8a9090]">Connected:</span>
              ...
              <div key={step.id} className="flex-1 flex flex-col items-center relative">
                ...
                {index < STEP_CONFIG.length - 1 && (
                  <div className={`absolute top-4 left-1/2 w-full h-0.5 ${
                    isComplete ? 'bg-green-500' : isActive ? 'bg-[#c5a97b]' : 'bg-[#1f2827]'
                  }`} />
                )}
                ...
                <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all ${
                  ...
                  isActive ? 'bg-[#b8860b] border-[rgba(255,208,117,0.4)] text-white animate-pulse' :
                  'bg-[#1a2221] border-[rgba(255,255,255,0.08)] text-[#8a9090]'
                }`}>
                  {isComplete ? '✓' : index + 1}
                </div>
      ...
      {statusMessage && (
        <div className={`p-3 rounded-lg text-sm ${
          ...
          'bg-[rgba(255,208,117,0.08)] border border-[rgba(255,208,117,0.2)] text-[#ffd075]'
        }`}>
          {isLoading && (
      ...
      {verification && (
        <div className={`p-4 rounded-lg border ${
          ...
          <p className="text-sm text-[#cecece]">{verification.reason}</p>
          {verification.tempImageUrl && (
            <p className="text-xs text-[#6b7575] mt-2">Image uploaded and ready for deployment</p>
          )}
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-[#cecece] mb-1">
            Token Image <span className="text-red-400">*</span>
            <span className="text-xs text-[#c5a97b] ml-2">(AppleSnakes style required)</span>
          </label>
          <div className="flex items-start gap-4">
            {imagePreview ? (
              <div className="relative">
                <img
                  src={imagePreview}
                  alt="Preview"
                  className="w-24 h-24 object-cover rounded-lg border border-[rgba(255,255,255,0.08)]"
                />
              ...
              <label className={`flex flex-col items-center justify-center w-24 h-24 border-2 border-dashed border-[rgba(255,255,255,0.08)] rounded-lg cursor-pointer hover:border-[rgba(255,208,117,0.4)] ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                <span className="text-2xl text-[#6b7575]">+</span>
                <span className="text-xs text-[#6b7575]">Upload</span>
                <input
                  type="file"
            ...
            <div className="text-xs text-[#6b7575] space-y-1">
              ...
              <p className="text-[#c5a97b]">Must match AppleSnakes art style</p>
            </div>
          </div>
        </div>
        ...
        {canVerify && (
          <button
            ...
            disabled={isLoading}
            className="w-full py-3 px-4 bg-gradient-to-r from-[#b8860b] to-[#c5a97b] text-white font-semibold rounded-lg hover:from-[#c5a97b] hover:to-[#ffd075] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  maxLength={32}
                  disabled={isLoading}
                  className="w-full px-3 py-2 bg-[#171e1d]/70 border border-[rgba(255,255,255,0.08)] rounded-lg text-white placeholder-[#6b7575] focus:outline-none focus:border-[rgba(255,208,117,0.4)] disabled:opacity-50"
                />
                <div className="text-xs text-[#6b7575] mt-1">{formData.name.length}/32</div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={10}
                  disabled={isLoading}
                  className="w-full px-3 py-2 bg-[#171e1d]/70 border border-[rgba(255,255,255,0.08)] rounded-lg text-white placeholder-[#6b7575] focus:outline-none focus:border-[rgba(255,208,117,0.4)] disabled:opacity-50 uppercase"
                />
                <div className="text-xs text-[#6b7575] mt-1">{formData.symbol.length}/10</div>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                disabled={isLoading}
                className="w-full px-3 py-2 bg-[#171e1d]/70 border border-[rgba(255,255,255,0.08)] rounded-lg text-white placeholder-[#6b7575] focus:outline-none focus:border-[rgba(255,208,117,0.4)] disabled:opacity-50 resize-none"
              />
            </div>
      ...
      {currentStep === 'complete' && launch && (
        <div className="p-4 rounded-xl border border-green-500/30 bg-green-950/20 space-y-4">
          ...
          {launch.contractAddress && (
            <div className="p-3 bg-[#1a2221]/50 rounded-lg">
              <p className="text-xs text-[#6b7575] mb-1">Contract Address</p>
            ...
            {launch.contractAddress && (
              <>
                <a
                  ...
                  className="flex-1 py-2 px-4 bg-[#1a2221] hover:bg-[#1f2827] text-[#c5a97b] text-center rounded-lg text-sm transition-colors"
                >
                  Dexscreener
                </a>
                <a
      ...
      {(currentStep === 'complete' || currentStep === 'error') && (
        <button
          onClick={handleReset}
          className="w-full py-2 px-4 border border-[rgba(255,255,255,0.08)] text-[#8a9090] rounded-lg hover:border-[rgba(255,255,255,0.08)] hover:text-[#cecece] transition-colors"
        >
      ...
      <div className="p-4 rounded-lg bg-[#171e1d]/50 border border-[rgba(255,255,255,0.08)] space-y-3">
        <div className="flex items-center justify-between text-sm">
          <span className="text-[#8a9090]">Your USDC Balance</span>
        ...
        <div className="border-t border-[rgba(255,255,255,0.08)] pt-3 space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="text-[#6b7575]">Verification + Upload</span>
            <span className="text-[#8a9090]">$0.50</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-[#6b7575]">Dev Buy Budget</span>
            <span className="text-[#8a9090]">${devBuyAmount.toFixed(2)}</span>
          </div>
          <div className="flex items-center justify-between text-sm font-semibold pt-2 border-t border-[rgba(255,255,255,0.08)]">
            <span className="text-[#cecece]">Total Cost</span>
            <span className="text-[#c5a97b]">${totalCost.toFixed(2)} USDC</span>
          </div>
        </div>
      </div>
      ...
      <div className="text-xs text-[#6b7575] space-y-1">
        ...
        <p className="text-[#c5a97b]">Images must match AppleSnakes art style to pass verification.</p>
      </div>
    </div>
  );
}
